...
export class ScrollbarDrag {
  ...
  compute(o: number): void {
    ...
    this.state.isScrollbarVisible = true

    const target = clamp(this.sizes.height * (o / h), 0, this.sizes.max)

    this.state.vsPosition = target
    setTimeout(() => (this.state.isScrollbarVisible = false), 0)
  }

  update(e: IEvent): void {
    if (!this.state.disabled) {
      const o = e.clientY || e.targetTouches[0]?.clientY
      this.compute(o)
    }
  }
  ...
  start(): void {
    ...
    this.state.isScrollbarVisible = false
